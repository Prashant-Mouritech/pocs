...
PDF_PATH = r"DNDi-Clinical-Trial-Protocol-BENDITA-V5.pdf"  # Update this path accordingly
...
# Function to store text embeddings in a FAISS index with a document-like structure
def get_vector_store(text_chunks):
    ...
    # Create an array to store each chunk's document-like structure with embeddings
    documents = []
    for i, chunk in enumerate(text_chunks):
        # Generate embedding for the text chunk
        embedding = embeddings.embed_query(chunk)

        # Create a document structure similar to Azure format
        document = {
            "id": str(i),
            "fileName": "DNDi-Clinical-Trial-Protocol-BENDITA-V5.pdf",  # Use the actual file name
            "content": chunk,
            "contentVector": embedding  # Store embedding as the vector
        }
        documents.append(document)
    
    # Extract only the text data for FAISS storage
    text_data = [doc["content"] for doc in documents]
    ...
    print("FAISS index with document structure created and saved locally.")
