...
def get_conversational_chain():
    ...
    # """
    # prompt_template = """
    # Answer the question with the relevant section reference and the related document(s) from the provided context.
    # If the answer is not available in the provided context, simply say, "Answer is not available in the context" and do not provide any incorrect answer.
    ...
    # Answer:
    # - Section Reference: [Provide the section reference here, e.g., 01 78 36]
    # - Related Document(s): [Provide the relevant documents here, e.g., 40.05.64.4.06.A]
    # """
    prompt_template = """
    Answer the question briefly (limit your response to 10 words). 
    Provide relevant section references and related document(s) if available. 
...
def user_input(user_question):
    ...
    docs = vector_store.similarity_search(user_question, k=1)  # Perform a similarity search on the user's question
    ...
    # Truncate the response to 10 tokens
    response_text = " ".join(response_text.split()[:10]) + "..." if len(response_text.split()) > 10 else response_text
...
def main():
    ...
    if user_question:
        with st.spinner("Generating a response..."):
            ...
            # Append the response, along with relevant documents, to the chat history
    ...
    if st.session_state.chat_history:
        ...
        for i, (question, response, docs) in enumerate(reversed(st.session_state.chat_history)):
            ...
            # Include the relevant documents and references
            for doc in docs:
