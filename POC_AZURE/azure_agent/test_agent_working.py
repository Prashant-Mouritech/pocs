...
def get_chat_response(user_question):
    try:
        ...
        #'SearchItemPaged' object is not subscriptable, is because the result returned from the
        # search_client.search method in search_embeddings is of type SearchItemPaged, which is an 
        # iterable but not directly indexable (like a list or dictionary).

        # prompt += f"{search_results[0]['content']}\n\n"
